...
<?php
    ...
    or die ("loi ket noi")
		...
		if(isset($_POST['username']) && isset($_POST['passwordNew'])&& isset($_POST['email']))
		{
			$username = $_POST['username'];
			$passwordNew = $_POST['passwordNew'];
            $email = $_POST['email'];
            $_SESSION['username'] = $username;
            if( $_SESSION['username'] = $row['tentk'] and $email = $row['email'])
			{
                $doipass = mysqli_query($conn,"UPDATE `taikhoan` SET  `mk`='$passwordNew'");
                echo '<script language="javascript">';
                echo "javascript:alert('Đã Cập Nhật Thành Công')";
                echo '</script>';
            }
            else
            {
                echo '<script language="javascript">';
                echo "javascript:alert('Không Trùng Tên Đăng Nhập Hoặc Mail')";
                echo '</script>';
